...
***What is This***
	- This program currently has eight modes of function:
		- Pixel Circle
		...
		- Oval
			- This mode is similar to Pixel Circle but instead of inputting one width, input two for the height and
			  width of the oval.
		-Spline
			- This mode creates a smoothed path based on a set of input points.  Right click to add point after point.
			  A series of connected black lines will be formed sequentially along with an estimated path in red.
			  Pressing Enter runs the operation forming pixels along the path.
			  ...
			  remain hidden. 
				...
				- Help
					- Clicking on this will open a pop-up that has the mapping of the function keys and the website for 
					  further updates.
				- Grid
					- Clicking on this will toggle ON or OFF a grid which allows for easier distinction between pixels.
					  This mode is most helpful for when an image is imported into the program.
		- Plans
